...
def fine_tune_radar(model, tokenizer, data, batch_size, DEVICE, epochs=3, ckpt_dir='./ckpt', three_classes=False, domain="CS"):
    ...
    model.save_pretrained(os.path.join(ckpt_dir, domain+'-radar-finetuned'))
...
def evaluate_model(model, tokenizer, data, DEVICE, no_auc=False):
    ...
    with torch.no_grad():
        for i in tqdm.tqdm(range(len(sentences)), desc="Evaluating"):
            try:
                inputs = tokenizer(sentences[i], return_tensors="pt", padding='max_length', truncation=True, max_length=512).to(DEVICE)
...
def run_radar(data, DEVICE, finetune: bool=False, no_auc: bool=False, ckpt_dir = './ckpt', test_only: bool=False, three_classes: bool=False, domain: str="transfer"):
    ...
    if finetune and not test_only:
        ...
        fine_tune_radar(model, tokenizer, data, batch_size=12, DEVICE=DEVICE, epochs=3, ckpt_dir = ckpt_dir, three_classes=three_classes, domain=domain)
    elif finetune and test_only:
        model = AutoModelForSequenceClassification.from_pretrained(os.path.join(ckpt_dir, domain + '-radar-finetuned')).to(DEVICE)
